...
def test_generate_docs_creates_markdown(tmp_path, monkeypatch):
    ...
    assert out.parent == tmp_path / "reports", (
        "generate_docs must write to cfg.reports.output_dir, not a hardcoded path"
    )
...
# ---------------------------------------------------------------------------
# run() — staleness threshold branching
#
# These tests exercise the if/elif chain in run() that converts the integer
# returned by _check_staleness() into either: profile excluded (hard), warn
# but keep (soft), or silently use (fresh).  All external I/O is mocked so
# the tests remain unit-level.
# ---------------------------------------------------------------------------

from roebuck.analysers.pr import run as _run_pr


def _make_pr_run_cfg(
    stale_commit_threshold: int = 20,
    hard_stale_threshold: int = 100,
) -> AppConfig:
    return AppConfig(
        github=GitHubConfig(token="ghp_test", repo="owner/repo"),
        profile=ProfileConfig(
            patterns=["src/**/*.py"],
            stale_commit_threshold=stale_commit_threshold,
            hard_stale_threshold=hard_stale_threshold,
        ),
    )


def _make_profile_obj() -> ProjectProfile:
...
def _run_pr_mocked(tmp_path, monkeypatch, *, ahead_by: int | None):
    """Call run() with all external dependencies mocked.

    Returns a dict with the kwargs captured from build_user_prompt and
    _build_sections so callers can assert on profile and staleness_warning.
    """
    monkeypatch.chdir(tmp_path)
    cfg = _make_pr_run_cfg()
    profile_obj = _make_profile_obj()

    captured: dict = {}

    def fake_build_user_prompt(pr, specs, context=None, profile=None):
        captured["prompt_profile"] = profile
        return "mock user prompt"

    def fake_build_sections(pr, r, staleness_warning=None):
        captured["staleness_warning"] = staleness_warning
        return [("Summary", "ok")]

    with (
        patch("roebuck.analysers.pr.GitHubClient") as MockGH,
        patch("roebuck.analysers.pr.SpecLoader") as MockSpecLoader,
        patch("roebuck.analysers.pr.ClaudeClient") as MockClaude,
        patch("roebuck.analysers.pr.MarkdownReportWriter") as MockWriter,
        patch("roebuck.analysers.pr._load_profile", return_value=profile_obj),
        patch("roebuck.analysers.pr._check_staleness", return_value=ahead_by),
        patch("roebuck.analysers.pr.build_user_prompt", side_effect=fake_build_user_prompt),
        patch("roebuck.analysers.pr._build_sections", side_effect=fake_build_sections),
    ):
        MockGH.return_value.get_pr.return_value = _make_pr_data()
        MockSpecLoader.return_value.load_specs.return_value = {}
        MockClaude.return_value.analyse.return_value = _make_pr_result()
        MockWriter.return_value.write.return_value = tmp_path / "report.md"
...
def test_run_hard_threshold_excludes_profile(tmp_path, monkeypatch):
    """When ahead_by >= hard_stale_threshold, profile is excluded from the prompt."""
    captured = _run_pr_mocked(tmp_path, monkeypatch, ahead_by=100)  # == hard threshold

    assert captured["prompt_profile"] is None
    assert captured["staleness_warning"] is not None
    assert "excluded" in captured["staleness_warning"].lower()


def test_run_soft_threshold_warns_but_keeps_profile(tmp_path, monkeypatch):
    """When soft <= ahead_by < hard, a staleness warning is set but the profile is kept."""
    captured = _run_pr_mocked(tmp_path, monkeypatch, ahead_by=20)  # == soft threshold

    assert captured["prompt_profile"] is not None
    assert captured["staleness_warning"] is not None
    assert "stale" in captured["staleness_warning"].lower()


def test_run_below_soft_threshold_no_warning(tmp_path, monkeypatch):
    """When ahead_by < soft_stale_threshold, the profile is used with no staleness warning."""
    captured = _run_pr_mocked(tmp_path, monkeypatch, ahead_by=5)

    assert captured["prompt_profile"] is not None
    assert captured["staleness_warning"] is None
